...
def draw_line(masked_image,lines):
    ...
    for line in lines:
        for x1,y1,x2,y2 in line:
            cv2.line(lines_img,(x1,y1),(x2,y2),(0,255,0), thickness=3)
    return lines_img
...
def region(image):  # logic function
    ...
    lines = cv2.HoughLinesP(masked_image, rho=2,theta=np.pi/180, threshold=50,lines=np.array([]),minLineLength=40,maxLineGap=150) # it's a complex algo go read about it can't really tell much in comments
...
def fun(image):
    ...
    image=cv2.Canny(image,10,50) #edge di..
    image=region(image)
